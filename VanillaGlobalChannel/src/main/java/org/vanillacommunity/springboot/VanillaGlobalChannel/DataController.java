...
public class DataController {
    ...
    public Boolean getDataPack(@PathVariable int channelID,@PathVariable String serverDisplayName,
        ...
        return ChannelManager.instance.sendDataPack(dataPack);
    }
    @ResponseBody
    @RequestMapping("/testInfo/{channelID}")
    public String getChannelQueueInfo(@PathVariable int channelID)
    {
        Channel channel = ChannelManager.instance.channelMap.get(channelID);
    ...
    {
        List<String> resultList = new ArrayList<>();
        for(Channel channel: ChannelManager.instance.channelMap.values())
        {
            resultList.add(channel.getId()+"/"+channel.getDisplayName());
        }
        return resultList.toString();
    }
}
