        ...
        else if(args.length <= 1)
        {
            ...
            else if(args[0].equalsIgnoreCase("channel"))
            {
                ...
                if(playerData.setChannelID(channelID))
                {
                    ...
                    str = str.replaceAll("%channelName%",ChannelManager.getInstance().getChannelInfoMap().get(channelID));
                    p.sendMessage(Component.text(str));
                }
                else
                {
        ...
        else if(args.length <= 2)
        {
            if(args[0].equalsIgnoreCase("channel"))
            {
                try
                {
                    ...
                    if(!ChannelManager.getInstance().getChannelInfoMap().containsKey(channelID))throw new NumberFormatException("ignore");
